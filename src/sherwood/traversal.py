...
from typing import Callable, Deque, Iterator, Optional, Union, cast
...
def breadth_first(tree: Treelike) -> Iterator[Node]:
    """Rank-ordered breadth-first traverser based on a FIFO-queue."""

    def _traverser(node: Node) -> Iterator[Node]:
        nodes = deque([node])
        while nodes:
            node = nodes.popleft()
            yield node.value
...
def ordered_iterative(tree: Treelike) -> Iterator[Node]:
    """In-order depth-first-search, implemented as iterative generator."""

    def _traverser(start: Node) -> Iterator[Node]:
        backtracking = False
...
def depth_first_inorder(tree: Treelike) -> Iterator[Node]:
    """In-order depth-first-search, implemented as recursive generator."""

    def _in_order_traverser(node: Node) -> Iterator[Node]:
        if node.left is not None:
...
def depth_first_preorder(tree: Treelike) -> Iterator[Node]:
    """Pre-order depth-first-search, implemented as recursive generator."""

    def _pre_order_traverser(node: Node) -> Iterator[Node]:
        yield node.value
        if node.left is not None:
...
def depth_first_postorder(tree: Treelike) -> Iterator[Node]:
    """Post-order depth-first-search, implemented as recursive generator."""

    def _post_order_traverser(node: Node) -> Iterator[Node]:
        if node.left is not None:
...
def _traverse_tree_or_node(
    ...
    """Applies the traverser to the tree's root, or the given nodeing node."""
